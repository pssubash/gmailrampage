...
$totalRecords = "SELECT count(id) FROM emailgmail_6";
$resultSet = mysql_query ( $totalRecords, $dbLink );
$resultSet = mysql_fetch_array ( $resultSet );
$totalPermutions = $resultSet[0];

$totalRecords = "SELECT count(id) FROM emailgmail_6 where status = 1";
$resultSet = mysql_query ( $totalRecords, $dbLink );
$resultSet = mysql_fetch_array ( $resultSet );
...
$totalRecords = "SELECT count(id) FROM validemail_6";
$resultSet = mysql_query ( $totalRecords, $dbLink );
$resultSet = mysql_fetch_array ( $resultSet );
$totalValid= $resultSet[0];

$totalRecords = "SELECT count(id) FROM invalidemail_6";
$resultSet = mysql_query ( $totalRecords, $dbLink );
$resultSet = mysql_fetch_array ( $resultSet );
$totalInvalid= $resultSet[0];

echo "<br/>Total Permutaions of 6 character @gmail.com - ".$totalPermutions;
echo "<br/>Number of 6 character @gmail.com which have undergone verification -  ".$totalVerificationCompleted;
echo "<br/>Number of 6 character @gmail.com which needs to undergo verification -".$totalVerificationPending;
