...
$validator->localhost = REMOTE_SERVER_DOMAIN;
...
<?php } ?>
	<?php 
		$dbLink = mysql_connect ( REMOTE_HOST, DB_USERNAME, DB_PASSWORD ) or die ( "Error " . mysqli_connect_error (  ) );
		...
		$page = 0;
		$offset = NUM_VERIFICATION_PER_RUN;
		$count_valid = 0;
		...
		for($i = 0; $i < 1; $i ++) {
			if ($page > 1) {
				...
				$slno = $count_invalid + $count_valid;
			}
			...
			while ( $row = mysql_fetch_array ( $resultSet ) ) {
				...
				if ($result == 1) {
					...
					$count_valid++;
					if(strtolower(IS_SEND_EMAIL_ADVERISEMENT_TO_VERIFIED) == 'yes') {
						require_once 'send_ad.php';
					}
	...
	print "<br/>".NUM_VERIFICATION_PER_RUN." -  permutations of ".VERIFY_LETTER." characters verified in this run";
